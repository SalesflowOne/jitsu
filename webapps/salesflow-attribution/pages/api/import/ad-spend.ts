...
import { parseAdSpendCsv, persistAdSpendRows } from "@salesflow/attribution-core";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { csv, platform } = req.body as { csv?: string; platform?: string };
  ...
  try {
    const rows = parseAdSpendCsv(orgId, csv, platform);
    const count = await persistAdSpendRows(dbClient, rows);
